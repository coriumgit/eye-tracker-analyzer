...
ERROR_MSG_ONSET_AND_OFFSET_TRIGGERS_NR_DONT_AGREE = 'Onset and offset triggers must match';
...
%=================%
    ...
    function createEtasFromEyeTrackerFilesBtnCallback(~,~)          
        ...
        for requested_eta_i= 1:requested_etas_nr
            ...
            [curr_subject_eta_save_path, curr_subject_eta_save_file_name, ~]= fileparts(get(curr_subject_eta_save_file_etext, 'string'));
            ...
            curr_eta.save(curr_subject_eta_save_path);
    ...
    function saveFolderBtnCallback(~,~)
        ...
        ETA_FILES_DESTINATION= fullfile(FILES_SAVE_DESTINATION, ETAS_FOLDER_NAME);
...
%         end
    ...
    function [subjects_figs, statistisized_figs, analysis_struct_with_results]= analyzeMicrosaccades(subjects_etas, progress_screen)        
        ...
        [eye_data_struct, analysis_structs, eyeballing_stats]= saccades_extractor.extractSaccadesByEngbert( ...
            ...
            PERFORM_EYEBALLING, ...
            ETA_FILES_DESTINATION, 0.2, progress_screen);    
        ...
        function saveUpdatedEegStructs(progress_contribution, progress_screen)
            for subject_i= 1:subjects_nr
                ...
                pop_saveset(EEG, 'filename', [subject_eta.getAnalysisTag(), '.set'], 'filepath', ETA_FILES_DESTINATION);                
        ...
        function reformated_analysis_structs= reformatAnalysisStruct()
            ...
            for subject_i= 1:subjects_nr
                ...
                for cond_i= 1:numel(curr_subject_conds_names)
                    ...
                    reformated_analysis_structs{subject_i}.(curr_subject_conds_names{cond_i}).onsets= NaN(curr_cond_trials_nr, max_trial_dur);                    
                    ...
                    for trial_i= 1:curr_cond_trials_nr
                        ...
                        if ~isempty(curr_trial_saccades_struct.onsets) && any( ~isnan(curr_trial_saccades_struct.onsets) )
                            reformated_analysis_structs{subject_i}.(curr_subject_conds_names{cond_i}).onsets(trial_i, curr_trial_saccades_struct.onsets)= 1;                            
            ...
            for cond_i = 1:conds_nr
                ...
                for trial_i = 1:trials_nr
                    ...
                    fixations_struct = getFixationsFromSaccadesDetection(d, ...
                        ...
                        saccades_structs{subject_i}.(cond)(trial_i).amplitudes, 20, BLINKS_DELTA);
                    ...
                    fixations_coords = [fixations_struct.Hpos; fixations_struct.Vpos]';
...
%                     end
                    analysis_struct{subject_i}.(cond)(trial_i).fixations_onsets = fixations_struct.onsets;
                    analysis_struct{subject_i}.(cond)(trial_i).fixations_coordinates = fixations_coords;
                    analysis_struct{subject_i}.(cond)(trial_i).fixations_durations = fixations_struct.durations;                    
...
%         legend(TRIAL_ONSET_TRIGGERS{:});
    ...
    function etas= loadEtasSegmentized(progress_screen)     
        ...
        for subject_i= 1:subjects_nr                                     
            ...
            eta_loaded_struct= load(etas_files_list{subject_i}, '-mat');
            eta= eta_loaded_struct.eta;
            progress_screen.addProgress(0.5/subjects_nr);
            was_previous_segmentation_loaded= eta.segmentizeData(progress_screen, 0.5/subjects_nr, TRIAL_ONSET_TRIGGERS, TRIAL_OFFSET_TRIGGERS, TRIAL_REJECTION_TRIGGERS, BASELINE, POST_OFFSET_TRIGGERS_SEGMENT, TRIAL_DURATION, BLINKS_DELTA);
            if was_previous_segmentation_loaded
                ...
                eta.save(ETA_FILES_DESTINATION);
            end
    ...
    function has_succeeded= createOutputFolders()
        ...
        if exist(ETA_FILES_DESTINATION, 'dir')~=7
            mkdir(ETA_FILES_DESTINATION);        
        end
